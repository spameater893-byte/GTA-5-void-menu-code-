...
namespace VoidEngine {
    ...
    public:
        ...
        void Title(const char* title) {
            ...
            // Updated drawing native to match the strict function signature
            GRAPHICS::DRAW_RECT(0.15f, 0.12f, 0.22f, 0.08f, hR, hG, hB, hA, false);
            HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT(const_cast<char*>("STRING"));
            HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(const_cast<char*>(title));
            HUD::SET_TEXT_FONT(1);
        ...
        bool MenuOption(const char* option, const char* targetMenu) {
            ...
            GRAPHICS::DRAW_RECT(0.15f, yPos + 0.015f, 0.22f, 0.04f, isHighlighted ? hiR : bgR, isHighlighted ? hiG : bgG, isHighlighted ? hiB : bgB, isHighlighted ? hiA : bgA, false);
            HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT(const_cast<char*>("STRING"));
            HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(const_cast<char*>(option));
            HUD::SET_TEXT_FONT(0);
        ...
        bool BoolOption(const char* option, bool* toggle) {
            ...
            HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT(const_cast<char*>("STRING"));
            HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(*toggle ? const_cast<char*>("[ON]") : const_cast<char*>("[OFF]"));
            HUD::SET_TEXT_FONT(0);
            HUD::SET_TEXT_SCALE(0.0f, 0.35f);
...
void KeyboardInputHelper() {
    MISC::DISPLAY_ONSCREEN_KEYBOARD(1, const_cast<char*>("FMMC_KEY_TIP8"), const_cast<char*>(""), const_cast<char*>(""), const_cast<char*>(""), const_cast<char*>(""), const_cast<char*>(""), 32);
...
void DrawMenuText(const char* text, int font, float x, float y, float scaleX, float scaleY, int r, int g, int b, int a, bool center) {
    HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT(const_cast<char*>("STRING"));
    HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(const_cast<char*>(text));
...
void scriptTick() {
    ...
    if (menuOpen) {
        ...
        if (menu.CurrentMenu() == "weaponsub") {
            ...
            if (menu.Button("Give All Weapons")) {
                ...
                WEAPON::GIVE_DELAYED_WEAPON_TO_PED(p, MISC::GET_HASH_KEY(const_cast<char*>("WEAPON_CARBINERIFLE")), 9999, true);
                WEAPON::GIVE_DELAYED_WEAPON_TO_PED(p, MISC::GET_HASH_KEY(const_cast<char*>("WEAPON_RPG")), 9999, true);
            }
    ...
    if (moneyDrop) {
        Vector3 c = ENTITY::GET_ENTITY_COORDS(playerPed, true); OBJECT::CREATE_AMBIENT_PICKUP(MISC::GET_HASH_KEY(const_cast<char*>("PICKUP_MONEY_CASE")), c.x, c.y, c.z + 0.5f, 0, 25000, 0, false, true);
}
if (noMissionFailure) {
